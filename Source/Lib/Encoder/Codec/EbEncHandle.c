...
EbErrorType load_default_buffer_configuration_settings(
    ...
    /******************************************************************
    * Plateform detection, limit cpu flags to hardware available CPU
    ******************************************************************/
    const CPU_FLAGS cpu_flags = get_cpu_flags();
    sequence_control_set_ptr->static_config.use_cpu_flags &= cpu_flags;
#ifdef NON_AVX512_SUPPORT
    /* Disable bits equal, or upper that CPU_FLAGS_AVX512F */
    sequence_control_set_ptr->static_config.use_cpu_flags &= (CPU_FLAGS_AVX512F - 1);
#endif
